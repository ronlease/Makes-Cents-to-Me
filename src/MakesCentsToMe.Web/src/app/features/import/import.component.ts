...
import { ApiService, CsvPreview, ImportProfile, ImportResult } from '../../services/api.service';
    ...
    <div class="import-container">
      <mat-stepper linear #stepper>
          ...
          <div class="step-content">
            @if (!csvPreview()) {
              ...
              <div class="preview-section">
                ...
                <div class="chip-row">
                  @for (header of csvPreview()!.detectedHeaders; track header) {
                ...
                <div class="table-container">
                  <table mat-table [dataSource]="csvPreview()!.previewRows" class="preview-table">
                    @for (header of csvPreview()!.detectedHeaders; track header; let colIndex = $index) {
                      <ng-container [matColumnDef]="header">
                        <th mat-header-cell *matHeaderCellDef>{{ header }}</th>
                        <td mat-cell *matCellDef="let row">{{ row[colIndex] }}</td>
                      </ng-container>
                    }
                    <tr mat-header-row *matHeaderRowDef="csvPreview()!.detectedHeaders"></tr>
                    <tr mat-row *matRowDef="let row; columns: csvPreview()!.detectedHeaders;"></tr>
                  </table>
                </div>
        ...
        <mat-step label="Process">
          <div class="step-content">
            @if (!existingProfile() || !savedProfile()?.balanceProvided) {
...
export class ImportComponent implements OnInit {
  protected readonly accountId = signal<number>(0);
  ...
  protected readonly csvPreview = signal<CsvPreview | null>(null);
  ...
  get backRoute(): () => string[] {
    return () => {
      ...
      if (profile) {
        return ['/institutions', String(profile.accountId), 'accounts'];
      }
      return ['/institutions'];
    };
  }
  ...
  advanceFromUpload(): void {
    const headers = this.csvPreview()?.detectedHeaders ?? [];
  ...
  ngOnInit(): void {
    ...
    this.accountId.set(idParam ? Number(idParam) : 0);
  }
  ...
  processImport(): void {
    ...
    this.apiService.processImport(this.accountId(), request).subscribe({
      next: result => {
  ...
  saveProfile(): void {
    ...
    for (const header of this.detectedHeaders()) {
      columnMappings[header] = this.mappingForm.controls[header]?.value ?? 'Ignore';
    }
    const request = {
  ...
  private buildMappingForm(headers: string[]): void {
    ...
    for (const header of headers) {
      controls[header] = new FormControl<string>('Ignore', { nonNullable: true });
    }
